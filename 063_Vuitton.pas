...
var
    ...
    banyak: integer; //banyak matkul
    i,j: integer;
    total_sks: real;
    panjang: integer; //length
    f: TextFile;

//function konversi huruf menjadi angka
...
begin
    ...
    for i:= 1 to banyak do
        begin
            ...
            if panjang>0 then 
                begin
                    panjang:= length(mahasiswa.huruf[i]); 
                end;
    ...
    //Hasil Output
    clrscr;
    assign(f,'KHS_231401063_Vuitton Varian Utomo.txt');
    rewrite(f);
    ...
    begin
        write(f, '|',i,'. |',matkul[i].nama);
            for j:= 1 to (45 + panjang - length(matkul[i].nama)) do 
            begin
                write(f, ' '); 
            end;

        write(f, '|',mahasiswa.huruf[i]);
            for j:= 1 to (5 - length(mahasiswa.huruf[i])) do 
            begin
                write(f, ' '); 
            end;
